...
class DroneEKF:
    def __init__(self, dt=0.1, model='input', measurement_names=('psi', 'r_x', 'r_y', 'a_x', 'a_y')):
        """ Initialize the EKF.
    ...
    def run_ekf(self, x0, u_sim, y_sim, P_init=None, Q_init=1e-4, R_sim=None):
        ...
        R_default_dict = {'psi': 1e-1, 'gamma': 1e-1, 'beta': 1e-1,
                          ...
                          'a': 1e-1, 'r': 1e-1, 'g': 1e-1, 'z': 1e-1, 'zeta': 1e10,
        ...
        for k in range(1, w):  # each time-step
            ...
            # Simulate measurement & add some noise
            z = y_sim.values[k, :]
        ...
        x_est['g'] = np.sqrt(x_est['v_x'].values ** 2 + x_est['v_y'].values ** 2)
        x_est['beta'] = np.arctan2(x_est['v_y'].values, x_est['v_x'].values)
    ...
    def simulate(self, x, u):
        ...
        for k in range(0, u.shape[0] - 1):
            ...
            # New state
            x_k = self.f(x_sim[k], u_k)

            # Update the wind states with the new state because model doesn't account for their change
            for state_name in ('w_x', 'w_y', 'w_x_dot', 'w_y_dot'):  # wind state names
                state_index = self.state_names.index(state_name)
                x_k[state_index] = x.loc[k + 1, state_name]

            # New measurement
            y_k = self.h(x_k, u_k)

            # Store
            t_sim.append((k + 1) * self.dt)
            x_sim.append(x_k)
            u_sim.append(u_k)
            y_sim.append(y_k)

        x_sim = pd.DataFrame(np.vstack(x_sim), columns=self.state_names)
        u_sim = pd.DataFrame(np.vstack(u_sim), columns=self.input_names)
        y_sim = pd.DataFrame(np.vstack(y_sim), columns=self.measurement_names)
    ...
    def f(self, X, U):
        """ Discretize the dynamics with simulation to next step.
        """

        def wrapped_dynamics(t, x):
            return self.f_continuous(x, U)

        # Integrate
        sol = scipy.integrate.solve_ivp(wrapped_dynamics, [0, self.dt], X, method='RK45', t_eval=[self.dt])
        X_k = sol.y[:, -1]
